...
export class CitasComponent implements OnInit {
  ...
  confirmarLlegada(id: number | undefined): void {
    ...
    this.citaService.confirmarCita(id).subscribe({
      next: () => { this.toast.success('Cita confirmada correctamente.'); this.onFiltrar(); },
  ...
  cancelarCita(id: number | undefined) {
    ...
    this.citaService.cancelarCita(id).subscribe({
      next: () => { this.toast.success('Cita cancelada correctamente.'); this.onFiltrar(); },
  ...
  confirmarReprogramar(cita: Cita): void {
    if (!this.nuevaFechaRep || !this.nuevaHoraRep) {
      this.toast.warning('Ingrese la nueva fecha y hora.');
      return;
    }
    this.citaService.reprogramarCita(cita.id_cita!, this.nuevaFechaRep, this.nuevaHoraRep).subscribe({
      next: () => { this.citaReprogramandoId = null; this.toast.success('Cita reprogramada.'); this.onFiltrar(); },
      error: () => this.toast.error('Error al reprogramar la cita.')
    });
  }
